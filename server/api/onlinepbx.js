...
class OnlinePBX {
    ...
    async authenticate() {
        try {
            // Если ключ уже содержит ':', значит это key_id:key, используем его напрямую
            if (this.apiKey && this.apiKey.includes(':')) {
                this.authKey = this.apiKey;
                console.log('Using provided API key credential');
                return this.authKey;
            }
            ...
            const response = await axios.post(
                ...
                {
                    headers: {
                        'x-pbx-authentication': this.apiKey,
            ...
            if (response.data.status === 'ok' || response.data.status === true) {
                this.authKey = response.data.data?.key_id
                    ? `${response.data.data.key_id}:${response.data.data.key}`
                    : this.apiKey;
            ...
            // Если авторизация не удалась, пробуем использовать исходный ключ
            this.authKey = this.apiKey;
            return this.authKey;
        }
    }
    ...
    async makeCallInstantly(from, to) {
        try {
            ...
            if (response.data.status === 'ok' || response.data.status === true || response.data.status === '1') {
                return {
                    success: response.data.status === 'ok' || response.data.status === true || response.data.status === '1',
            ...
            console.error('OnlinePBX makeCallInstantly error:', error.response?.data || error.message);
            return {
                success: false,
                error: error.response?.data?.comment || error.message
            };
