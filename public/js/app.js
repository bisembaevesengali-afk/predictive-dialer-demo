...
function checkAuth() {
    ...
    // Обновляем UI под пользователя (например, аватарку или настройки)
    updateUserSettings();
}
...
function updateUserSettings() {
    ...
    if (extInput) extInput.value = currentUser.extension;
}
...
// responsible_user_id: 100 (Менеджер А), 200 (Менеджер Б)
const leadPool = [
    {
    ...
    {
        id: 4, contactName: 'Melody Simpson', phone: '+1 415 555 9876', price: '150', date: '09.02.2023', timestamp: 1675900800000,
        status: 'pending', pipeline: 'Продажи', stage: 'Принимают решение', responsible_user_id: 200,
        calls: []
    },
    {
        id: 5, contactName: 'Айгерим Касимова', phone: '+7 707 444 88 99', price: '89 000', date: 'Сегодня', timestamp: Date.now() - 3600000,
        status: 'pending', pipeline: 'Повторные продажи', stage: 'Переговоры', responsible_user_id: 100,
        calls: [
            { date: 'Сегодня 10:00', status: 'success', duration: '01:20' }
        ]
    },
    {
        id: 6, contactName: 'Тимур Ибраев', phone: '+7 777 555 66 77', price: '300 000', date: 'Сегодня', timestamp: Date.now() - 300000,
        status: 'pending', pipeline: 'Продажи', stage: 'Первичный контакт', responsible_user_id: 200,
        calls: []
    },
    {
        id: 8, contactName: 'Иван Кузнецов', phone: '+7 747 161 61 63', price: '0', date: 'Вчера', timestamp: Date.now() - 90000000,
        status: 'pending', pipeline: 'Холодный обзвон', stage: 'Первичный контакт', responsible_user_id: 100,
        calls: [
            { date: 'Вчера 09:00', status: 'no-answer', duration: '00:00' }
        ]
    },
    {
        id: 10, contactName: 'Кевин Спейси', phone: '+7 777 161 61 63', price: '99 000', date: 'Только что', timestamp: Date.now(),
        status: 'pending', pipeline: 'Холодный обзвон', stage: 'Переговоры', responsible_user_id: 200,
        calls: []
    }
];
...
window.switchTab = function (tabName) {
    ...
    if (targetTab) {
        ...
        // Если это дайлер, возвращаем сетку
        if (tabName === 'dialer') {
            targetTab.classList.add('grid');
        } else {
            targetTab.classList.remove('grid');
        }
    }
...
// --- ИНИЦИАЛИЗАЦИЯ ---
let amocrmPipelines = []; // Сохраняем структуру воронок

document.addEventListener('DOMContentLoaded', async () => {
    // 1. Сначала проверяем авторизацию
    if (!currentUser) return;

    // 2. Загружаем воронки и этапы из AmoCRM
    await loadAmoPipelines();

    applySorting();
    renderQueue();

    // Показываем первого из очереди по умолчанию
    ...
    // Кнопка добавления в очередь из карточки
...
async function loadAmoPipelines() {
    try {
        ...
        if (pSelect) {
            ...
            // Слушатель смены воронки для подгрузки этапов
...
function handleLiveSearch(query) {
    ...
    if (dealMatches.length > 0) {
        renderSection(content, 'СДЕЛКИ', dealMatches);
    }

    if (phoneMatches.length > 0) {
        const uniquePhoneMatches = phoneMatches.filter(pm => !dealMatches.find(dm => dm.id === pm.id));
        if (uniquePhoneMatches.length > 0) {
            renderSection(content, 'КОНТАКТЫ', uniquePhoneMatches, true);
        }
    }
...
window.applyFilters = async function () {
    ...
    try {
        // Запрашиваем реальные сделки у сервера
        // Если выбран этап или воронка, фильтруем по ним сразу в API
        let url = `/api/amocrm/leads?limit=250`;
        if (pipelineId !== 'all') url += `&pipeline_id=${pipelineId}`;
        if (stageId !== 'all') url += `&status_id=${stageId}`;

        // Добавляем фильтр по текущему менеджеру (если не админ)
        if (currentUser && currentUser.id !== 7751419) {
        ...
        // 1. Применяем маппинг этапов и фильтрацию
        filteredResults = realLeads.map(lead => {
            // Ищем название этапа в загруженных воронках
        ...
        if (pipelineId !== 'all') {
            ...
            filterText += `Воронка: ${p ? p.name : '?'}`;
        }
        if (stageId !== 'all') {
            const p = amocrmPipelines.find(p => p.id == pipelineId);
            const s = p?._embedded?.statuses?.find(s => s.id == stageId);
            filterText += (filterText ? ' | ' : '') + `Этап: ${s ? s.name : '?'}`;
        }
        if (filterText) input.value = filterText;
        ...
        tbody.innerHTML = '<tr><td colspan="4" class="p-10 text-center text-red-500 font-bold">Ошибка AmoCRM: Проверьте соединение</td></tr>';
    }
}
...
function renderQueue() {
    ...
    // СОРТИРОВКА: 
    // 1. Dialing (в самом верху)
    // 2. Pending (в очереди)
    // 3. Talked / No-Answer (в самом низу)
    const sortedList = [...queue].sort((a, b) => {
        const priority = { 'dialing': 0, 'pending': 1, 'talked': 2, 'no-answer': 2 };
        if (priority[a.status] !== priority[b.status]) {
            return priority[a.status] - priority[b.status];
        }
        // Внутри одной группы по дате (если нужно)
        return b.timestamp - a.timestamp;
    });

    const displayList = sortedList;

    // Считаем только тех, кому еще предстоит позвонить
    const pendingCount = queue.filter(l => l.status === 'pending' || l.status === 'dialing').length;
    counter.innerText = pendingCount;

    const totalBudget = queue
        .filter(l => l.status === 'pending' || l.status === 'dialing')
        .reduce((sum, lead) => sum + parsePrice(lead.price), 0);
    ...
    displayList.forEach(lead => {
        ...
        if (lead.status === 'dialing') {
            classes += "border-amber-400 bg-amber-50 shadow-lg z-10 ";
        } else if (lead.status === 'talked') {
            classes += "opacity-75 bg-slate-50 border-slate-100 ";
        } else if (lead.status === 'no-answer') {
            classes += "opacity-75 bg-red-50/30 border-red-100 ";
        } else {
            classes += "hover:border-teal-500/30 hover:shadow-xl ";
        }
        ...
        let statusMark = '';
        if (lead.status === 'dialing') {
            statusMark = '<span class="text-[10px] font-black text-amber-600 animate-pulse uppercase">Звоним...</span>';
        } else if (lead.status === 'talked') {
            statusMark = '<span class="text-[10px] font-black text-teal-600 uppercase"><i class="fa-solid fa-check-double mr-1"></i>Поговорили</span>';
        } else if (lead.status === 'no-answer') {
            statusMark = '<span class="text-[10px] font-black text-red-500 uppercase"><i class="fa-solid fa-phone-slash mr-1"></i>Нет ответа</span>';
        }
            ...
            <div class="flex justify-between items-start gap-2 mb-2">
                <div class="flex items-center gap-3 min-w-0">
                    <div class="relative shrink-0">
                        ...
                        ${lead.status === 'dialing' ? '<div class="absolute -top-1 -right-1 w-3 h-3 bg-amber-500 rounded-full animate-ping"></div>' : ''}
                    </div>
                    <div class="min-w-0">
...
function startSimulation() {
    ...
    document.getElementById('dialerStatusLine').classList.remove('hidden');
...
function pauseSimulation() {
    ...
    const statusLine = document.getElementById('dialerStatusLine');
    if (statusLine) statusLine.innerText = 'Обзвон на паузе';
}
...
function resumeSimulation() {
    ...
    const statusLine = document.getElementById('dialerStatusLine');
    if (statusLine) statusLine.innerText = 'Обзвон запущен...';
...
function stopSimulation() {
    ...
    const line = document.getElementById('dialerStatusLine');
    const bar = document.getElementById('floatingDialerBar');
    if (line) {
        line.classList.add('hidden');
        line.innerText = 'Обзвон в процессе...';
    }
    ...
    alert('Обзвон завершен. Статистика будет доступна в следующем обновлении.');
}
...
async function cycle() {
    ...
    if (!isSimulating || isPaused) {
        // Если была нажата пауза во время попытки, возвращаем статус
